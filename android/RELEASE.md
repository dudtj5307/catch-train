...
> 이 문서의 경로와 명령은 모두 **`android/` 기준**이다 (Gradle 루트).
...
`applicationId` 는 `dev.yslee.catchtrain`. release 빌드는 이 폴더(`android/`)의 `keystore.properties` 를
...
파일은 지우지 않고 `android/` 에 그대로 뒀다. 구 APK 의 업데이트를 낼 일이 생기면
그때만 쓴다. 그럴 계획이 없으면 삭제해도 된다.

키를 새로 만들어야 하면:

```bash
cd android && keytool -genkeypair -v -keystore catchtrain-release.jks -storetype PKCS12 -keyalg RSA -keysize 4096 -validity 10950 -alias catchtrain
```
...
```bash
cd android && apksigner verify --print-certs app/build/outputs/apk/release/app-release.apk
```
