...
/**
 * 페이지 갱신 결과. (DESIGN.md §10, §38-9)
 *
 * 갱신은 **페이지 새로고침**(`WebView.reload()`, 브라우저의 F5) 하나뿐이다.
 *
 * 예전에는 결과 화면의 [열차조회] 버튼 좌표에 진짜 터치를 내려보냈다. **모바일 폭에서는
 * 그 버튼이 존재하지 않는다.** `div.btnWrap.btn_box` 가 `display:none` 이라 rect 가
 * 0×0 이고, 옆에 보이는 것은 눌러선 안 되는 `다음날 (…) 조회` 뿐이다. (§38-9)
 * 누를 것이 없으니 "누르는 방식" 자체가 성립하지 않는다.
 *
 * 새로고침이 사용자의 조회 조건을 날리지 않는 이유는 조건이 DOM 이 아니라
 * `localStorage["LS_TICKET_GENERAL"]` 에 있기 때문이다. 다시 불러오면 SPA 가 그 값으로
 * 같은 조회를 스스로 되풀이한다. **그래서 대원칙 4 는 그대로다** — 앱은 여전히
 * 조회 조건을 갖지 않고, 사용자가 사이트에 넣어 둔 것을 그대로 다시 쓴다.
 *
 * 여전히 쓰지 않는 방법:
 *  - `loadUrl` 로 조회 URL 직접 호출 : 이 경로는 사실상 항상 차단된다.
 *  - JS `el.click()` / `dispatchEvent` : isTrusted=false 인 합성 이벤트라 구분된다.
 *
 * 값이 비싸진 것은 감수한다. 한 사이클이 문서 + 번들 + 조회 API 전체가 되어
 * AJAX 재조회보다 요청이 크다. **간격을 좁히지 말 것.** (대원칙 2)
 */
sealed interface PageOutcome {
    ...
    /**
     * 갱신이 **목록이 다시 그려진 상태까지** 갔다. 정상 경로다.
     *
     * 새로고침에서는 `onPageFinished` 만으로 부족하다. 코레일은 React SPA 라 문서를 받은
     * 뒤에 번들이 돌고 조회 API 를 쳐서 `li.tckList` 를 그린다. 그 렌더링까지 확인한
     * 결과가 이것이다. (예매 2단계·되돌리기에서는 화면 전환 없는 DOM 갱신을 뜻한다)
     */
    data class Updated(override val detail: String) : PageOutcome

    /**
     * 갱신은 했지만 정해진 시간 안에 목록이 다시 보이지 않았다.
     * 조회 결과가 실제로 0건일 수도 있으므로,
     * 곧바로 실패로 보지 않고 DOM 분석을 한 번 시도한다. (대원칙 6)
     */
     ...
     * 목표한 화면에 닿지 못했다.
     *
     * 갱신([PageHost.requery])은 새로고침 하나뿐이라 이 결과를 내지 않는다.
     * 지금은 [PageHost.dismissReserveResult] 가 **뒤로 가서도 열차 목록이 없을 때**만 쓴다.
     */
    data class ButtonNotFound(override val detail: String) : PageOutcome

    /**
     * **화면에 없는 WebView 라 아무것도 하지 않았다.**
     * WebView 가 창에서 떨어졌거나 숨겨졌거나 크기가 0 인 경우다.
     *
     * 사람이 볼 수 없는 화면을 새로고침하는 것은 사람의 조작에서 나올 수 없는
     * 요청이라 하지 않는다. (대원칙 1 의 따름정리)
     *
     * 요청이 나가지 않은 상태이므로 차단 위험은 없다.
     * 화면이 돌아오면 곧바로 회복되므로 재시도할 수 있다.
     */
     ...
     * 할 수 있었지만 **일부러 하지 않았다.**
     *
     * 지금은 팝업 창(달력 등)이 열려 있는 경우다. 새로고침은 사용자가 팝업에서
     * 고르던 것까지 통째로 날린다. 조건을 고치는 중에 갱신이 나가면 요청만 늘고
     * 사용자가 하던 일이 사라지므로 이번 차례를 건너뛴다.
     *
...
interface PageHost {
    ...
    /**
     * **페이지를 새로고침해**(F5) 같은 조건으로 다시 조회하고, 목록이 다시 그려질 때까지 기다린다.
     *
     * 조회 조건은 `localStorage` 에 남아 있어 새로고침해도 그대로다. (§38-9)
     *
     * @param timeoutMs 새로고침한 문서의 onPageFinished 를 기다리는 최대 시간
     * @param settleTimeoutMs 문서 로딩 뒤 SPA 가 목록을 다시 그리기를 기다리는 최대 시간
     * @param onClick 갱신 동작이 실제로 일어난 직후 호출된다. 무엇을 새로고침했는지 담긴다.
     */
    suspend fun requery(
        timeoutMs: Long,
    ...
    /**
     * **진단 전용.** 역 선택 창이 안 뜨는 원인을 한 줄로 돌려준다. (§38-10)
     *
     * 감시 루프는 이것을 부르지 않는다. 사람이 [로그] 창에서 눌렀을 때만 돈다.
     * 읽기만 하므로 요청이 나가지 않고, 대원칙 2 와도 무관하다.
     *
     * 기본 구현은 "진단 없음" 이다 — 테스트용 가짜 호스트까지 이걸 흉내 낼 이유가 없다.
     */
    suspend fun probeStationPopup(): String = "진단을 지원하지 않는 호스트"
}
