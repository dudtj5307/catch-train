...
**[예매] 를 가로막는 안내 창의 [확인] 까지는 누른다.** (§38-6-2) 열차에 따라 [예매] 와
결제 화면 사이에 안내 창이 하나 끼어들고, 그 [확인] 을 누르기 전에는 아무 일도
일어나지 않는다. 범위를 넓힌 것이 아니라 **[예매] 한 번이 두 번의 터치가 된 것**이다.
고르는 규칙은 2단계와 같다 — **제목·버튼 문구 완전일치**(`이용안내` / `확인`)로만 보고,
어긋나면 누르지 않고 `NOTICE_BLOCKED` 로 사람에게 넘긴다.
...
- **[예매] 뒤에 안내 창이 끼어드는 열차가 있다.** (2개 편성을 이어 붙여 운행하는 열차)
  그 창의 [확인] 을 누르기 전에는 결제 화면으로 넘어가지 않는다. 역 선택 창과 같은
  react-modal 이라 `document.body` 에 붙고, **목록 서명에도 MutationObserver 에도
  잡히지 않는다** — 그냥 기다리면 예산만 버리고 "화면이 안 바뀜" 으로 끝난다.
  그래서 2단계를 기다리는 폴링마다 창이 떴는지 읽고, **아는 창일 때만** 누른다. (§38-6-2)
  - **아는 창의 기준을 넓히지 말 것.** 예약 실패·차단 안내에도 [확인] 이 있는데
    그것은 조회 폼을 새로 여는 버튼이라 **사용자의 조회 조건을 날린다** (대원칙 5).
    제목(`이용안내`)과 버튼 문구(`확인`)를 **완전일치**로만 본다.
