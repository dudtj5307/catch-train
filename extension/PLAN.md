...
| 측정 | 무엇을 | 요청이 나가나 | 판단 |
|---|---|---|---|
| **M-a** | 조회 결과 화면에서 좌석 칸 `a` 에 `armConfirm` → `el.click()` | 아마 안 나감 (화면 내 동작). **Network 탭으로 확인할 것** | `active` 가 붙으면 → React 는 합성 이벤트를 받는다 = 1단계 synthetic 가능 |
...
| `chrome.alarms` | 최소 30초. 기본 간격이 0.1~0.3초인 제품에서 주 타이머가 될 수 없다 |
