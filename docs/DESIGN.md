...
#### 합성 클릭이 통하는가 (M-a) — 재는 도구는 둘이 되었다

**아직 실측 전이다.** 다만 **재는 길이 둘로 늘었다.**

1. 팝업 **[로그 보기] → [클릭 진단]** — 눌러 보기만 하고 [예매] 는 건드리지 않는다 (아래)
2. **그냥 감시한다** — 좌석이 열린 그 순간에 확장이 실제로 누르고,
   통했는지 확인해서 로그에 남긴다. 통하지 않으면 그 자리에서 멈추고 사람을 부른다.

2번이 가능해진 것은 감시 경로가 **누른 뒤 반드시 확인하기** 때문이다 (`content/ktx/reserve.js`).
`active` 가 붙지 않으면 2단계로 넘어가지 않으므로, 합성 클릭이 통하지 않는 사이트에서도
사고가 나지 않고 `RESERVE_FAILED=SELECT/SEAT_NOT_SELECTED` 한 줄이 남는다. 그 줄이
`NO_EFFECT` 와 같은 뜻이고, **그때 비로소 M-b(디버거)를 잴 값어치가 생긴다.**

로그로 읽는 법:

| 로그 | 뜻 |
|---|---|
| `RESERVE_SEAT_SELECTED` | **합성 클릭이 통했다.** = `SYNTHETIC_OK`. 디버거는 영영 안 써도 된다 |
| `RESERVE_FAILED=SELECT/SEAT_NOT_SELECTED` | 눌렀는데 안 골라졌다. = `NO_EFFECT` |
| `RESERVE_FAILED=SELECT/ROW_NOT_FOUND` 또는 `CELL_NOT_FOUND` | 클릭과 무관하다. 화면이 그새 바뀐 것 |
| `RESERVE_CLICKED` 의 `trusted=false` | **정상이다.** 확장의 클릭은 원래 그렇다 |

아래 [클릭 진단] 표는 그대로 유효하다. 진단은 [예매] 를 누르지 않으므로,
좌석을 실제로 잡을 생각이 없을 때 재는 길로 남는다.

크롬 확장의 content script 는 `el.click()` 밖에 못 쓰는데 그 이벤트는 `isTrusted=false` 다.
안드로이드가 `MotionEvent` 로 넘었던 벽이고, 확장에서 넘는 길은 `chrome.debugger` 하나뿐인데
상시 배너와 §38-10 의 매크로 감지 위험이 딸려 온다. 그래서 **사이트가 `isTrusted` 를
실제로 보는지 먼저 잰다.**

재는 것은 확장 팝업의 **[로그 보기] → [클릭 진단]** 이다 (`extension/src/content/ktx/probe.js`).
두 번 눌러야 돌고, `AVAILABLE` 인 좌석 칸 하나를 실제로 한 번 누른다. **[예매] 는 누르지
않는다** — 되돌릴 수 있는 자리까지만 간다. 진단하는 동안 나간 요청도 같이 세므로
**devtools 를 열 필요가 없다** (§38-10 감지를 건드리지 않는다).

| 재는 값 | 뜻 |
|---|---|
| `verdict` | `SYNTHETIC_OK` / `NO_EFFECT` / `NOT_DELIVERED` / `WRONG_CELL` / `CLICK_THREW` |
| `event.fired` `trusted` `onTarget` `prevented` | 이벤트가 페이지 리스너까지 갔는가 |
| `active` 전 → 후 | **정답지.** 1단계로 그 칸이 골라졌는가 (§38-6-1) |
| 하단바 전 → 후 | `ticket_reserv_wrap` 이 없다가 생기는가 / 버튼 문구가 무엇인가 |
| 요청 N건 | 좌석 칸 클릭이 화면 안에서 끝나는가, 서버로 나가는가 |
| `hit` | 진짜 포인터라면 그 좌표에 닿는가. **디버거 드라이버(M-b) 판단용**이고 위 판정과 별개다 |

`SYNTHETIC_OK` 면 디버거는 영영 안 써도 된다 — 배너도 감지 위험도 없는 가장 좋은 결말이다.
`NO_EFFECT` 면 그때 비로소 M-b(디버거를 붙이기만 하고 5분 두기)를 잴 값어치가 생긴다.

기록할 곳: 아래 한 줄에 결과를 적는다.

```
M-a 결과: (미측정)  일시: —  화면: —
```

#### 2단계 뒤에 실제로 뜨는 화면 (M-c) — 아직 값이 없다

`RESERVE_FAILED_MARKERS` 가 SRT 문구 그대로인 것과 짝인 문제다. **코레일에서 [예매] 를
누른 뒤 무엇이 뜨는지 우리는 모른다.** 그래서 감시는 판정을 추측하지 않고 이렇게 나눈다.

| 관찰한 것 | 결론 | 로그 |
|---|---|---|
| URL 이 바뀌거나 목록이 사라짐 | 눌렸고 화면이 넘어갔다 | `RESERVE_SUCCEEDED` |
| 실패 문구(마커)가 보임 | 잔여석없음. 뒤로 가서 감시 계속 | `RESERVE_SOLD_OUT` |
| **모르는 모달이 떴다** | **추측하지 않고 사람에게 넘긴다** | `RESERVE_HANDOVER=UNKNOWN_NOTICE` |
| 아무것도 안 바뀜 | 인계 | `RESERVE_HANDOVER=NO_CHANGE` |

그리고 어느 쪽이든 **화면에 실제로 뜬 안내 문구를 `RESERVE_NOTICE` 로 남긴다** (200자).
조회 조건이 섞이지 않게 모달(또는 목록이 사라진 화면의 머리글)만 담는다.
**이 줄이 `RESERVE_FAILED_MARKERS` 를 코레일 값으로 고칠 근거다.** 기록할 곳:

```
M-c 잔여석없음 문구: (미측정)     모달인가 화면 전환인가: (미측정)
M-c 성공 시 화면:     (미측정)     일시: —
```
...
**얼마나 무거운지 재 두었다 (2026-08-30, 확장 실사용 로그, 조회 결과 10편성):**

```
[17:03:18] RESEARCH_TRIGGERED=chrome.tabs.reload
[17:03:19] PAGE_UPDATED=목록 10편성        ← 새로고침 → 목록 재렌더까지 약 1초
[17:03:19] NEXT_RELOAD=271ms
```

**한 사이클 ≈ 새로고침 1초 + 대기.** 이 숫자가 중요한 이유는 하나다 —
**"대기 시간 = 요청 간격" 이 아니다.** 대기가 0.3초면 사이클은 0.3초가 아니라 1.3초이고,
대기를 0 으로 해도 분당 60회를 넘지 못하며, 대기를 두 배로 늘려도 요청 수는 절반이
되지 않는다. 그래서 사람에게 보여 줘야 하는 값은 간격이 아니라 **분당 요청 수**다.

실제로 이것을 놓쳐서 사고가 있었다. 확장의 기본 간격이 **0.1~0.3초**로 남아 있었는데,
그 값은 갱신이 가벼운 AJAX 재조회이던 **SRT 시절의 것**이다. 새로고침 갱신으로 바꾸면서
숫자만 그대로 딸려 왔고, 위 로그가 **분당 46회**로 돌고 있는 것을 보여 준다.
(고친 뒤: `scheduler.js` 의 `DEFAULT_MIN/MAX_INTERVAL_MS` = 300/500,
`TYPICAL_CYCLE_OVERHEAD_MS` = 1000, 팝업이 분당 요청 수를 상시 표시)

**갱신 방식을 바꾸면 간격 기본값도 같이 본다.** 둘은 한 몸이다.
